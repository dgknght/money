...
describe TransactionItemCreator do
  ...
  it 'is creatable with an account' do
  ...
  describe 'transaction_date' do
    ...
    it 'is a date, or a date-parsable string' do
      creator = TransactionItemCreator.new(checking, attributes)
  ...
  describe 'other_account_id' do
    ...
    it 'islong to the same entity as the creating account' do
  ...
  describe 'amount' do
    ...
    context 'for an asset account' do
      ...
      it 'is positive with a debit action' do
      ...
      it 'is negative with a credit action' do
    ...
    context 'for a liability account' do
      ...
      it 'is negative with a debit action' do
        transaction = FactoryGirl.create(:transaction, amount: 100, debit_account: account)
        transaction_item = transaction.items.select { |i| i.account == account }.first
        creator = TransactionItemCreator.new(transaction_item)
        expect(creator.amount).to be < 0
      ...
      it 'is positive with a credit action' do
        transaction = FactoryGirl.create(:transaction, amount: 100, credit_account: account)
        transaction_item = transaction.items.select { |i| i.account == account }.first
        creator = TransactionItemCreator.new(transaction_item)
        expect(creator.amount).to be > 0
  ...
  describe 'create' do
    it 'creates a new transaction item with valid attributes' do
      creator = TransactionItemCreator.new(checking, attributes)
      item = creator.create
    ...
    it 'returns null with invalid attributes' do
      creator = TransactionItemCreator.new(checking, attributes.without(:description))
      expect(creator.create).to be_nil
    end

    it 'adjusts credit and debit actions for negative amounts' do
    ...
    it 'adjusts credit and debit actions for the account type' do
  ...
  describe 'create!' do
    ...
    it 'raises InvalidStateError with invalid attributes' do
      creator = TransactionItemCreator.new(checking, attributes.without(:description))
  ...
  describe 'update' do
    ...
    it 'updates the specified transaction with the specified description' do
      creator = TransactionItemCreator.new(transaction_item, attributes)
      expect do
        creator.update
        transaction.reload
    ...
    it 'updates the specified transaction with the specified transaction date' do
      creator = TransactionItemCreator.new(transaction_item, attributes)
      expect do
        creator.update
        transaction.reload
    ...
    it 'updates the specified transaction item with the specified amount' do
      creator = TransactionItemCreator.new(transaction_item, attributes)
      expect do
        creator.update
